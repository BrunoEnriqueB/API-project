...
export class ProductsController {
  static async addProduct(req: Request, res: Response) {
    const { name, description, price, quantity, category, bar_code }: reqBody =
      req.body;
    if (
      !name ||
      !description ||
    ...
    if (!category) {
      return res
        .status(401)
        .json({ message: 'Digite a categoria do produto!' });
    }
    if (!bar_code) {
      return res
        .status(401)
        .json({ message: 'Digite o código de barras do produto!' });
    }
    if (!req.file) {
